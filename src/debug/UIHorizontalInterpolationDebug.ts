...
import type { UIHorizontalInterpolationConstraint } from "../core";
...
export class UIHorizontalInterpolationDebug implements UIConstraintDebug {
  ...
  constructor(public readonly constraint: UIHorizontalInterpolationConstraint) {
  ...
  private readonly onRendering = (renderer: WebGLRenderer): void => {
    ...
    const anchorCX = isUIPlaneElement(elementC)
      ? elementC.x + elementC.width * this.constraint.anchorC
      : elementC.x;
    ...
    this.markerGeometry.setFromPoints([
      new Vector3(anchorCX, midY - markerSize, 0),
      new Vector3(anchorCX, midY + markerSize, 0),
    ]);

    const nextText: string = "hi: " + this.constraint.t.toFixed(2);
    ...
    this.textSprite.position.set(anchorCX, midY + 20, 0);
  };
