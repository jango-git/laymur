...
import type { UIHeightConstraint } from "../core";
...
export class UIHeightDebug implements UIConstraintDebug {
  ...
  constructor(public readonly constraint: UIHeightConstraint) {
  ...
  private readonly onRendering = (renderer: WebGLRenderer): void => {
    ...
    const { x, y, oppositeY, height, centerX, centerY } = this.constraint.element;

    if (height <= 0) {
      return;
    }

    this.lineGeometry.setFromPoints([new Vector3(x, y, 0), new Vector3(x, oppositeY, 0)]);

    const nextText: string = "H: " + height.toFixed(0);
    ...
    this.textSprite.position.set(centerX - 20, centerY, 0);
  };
