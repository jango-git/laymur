...
import type { UIWidthConstraint } from "../core";
...
export class UIWidthDebug implements UIConstraintDebug {
  ...
  constructor(public readonly constraint: UIWidthConstraint) {
  ...
  private readonly onRendering = (renderer: WebGLRenderer): void => {
    ...
    const { x, y, oppositeX, width, centerX, centerY } = this.constraint.element;

    if (width <= 0) {
      return;
    }

    this.lineGeometry.setFromPoints([new Vector3(x, y, 0), new Vector3(oppositeX, y, 0)]);

    const nextText: string = "W: " + width.toFixed(0);
    ...
    this.textSprite.position.set(centerX, centerY - 20, 0);
  };
