...
import { UIPriority, UISolverWrapper } from "../wrappers/UISolverWrapper";
...
export abstract class UILayer extends Eventail {
  ...
  constructor(w: number, h: number) {
    ...
    this.xIndex = this.solverWrapper.createVariable(0, UIPriority.P0);
    this.yIndex = this.solverWrapper.createVariable(0, UIPriority.P0);
    this.wIndex = this.solverWrapper.createVariable(w, UIPriority.P0);
    this.hIndex = this.solverWrapper.createVariable(h, UIPriority.P0);
  }
  ...
  protected resizeInternal(width: number, height: number): void {
    this.solverWrapper.suggestVariableValue(this.wIndex, width);
    this.solverWrapper.suggestVariableValue(this.hIndex, height);
  ...
  protected clickInternal(x: number, y: number): void {
    for (const element of this.sceneWrapper.enumerateElements()) {
...
// export interface UILayerInterface {
//   solverWrapper: UISolverWrapper;
//   sceneWrapper: UISceneWrapper;
// }
